...
import { DBMS_CURRICULUM } from '../data/databaseSystemsCurriculum.js';

const LessonPlannerDB = ({ subject = 'Database Systems', userId }) => {
  ...
  const generatePlan = async () => {
    ...
    if (curriculum.length === 0) {
      curriculum = DBMS_CURRICULUM.nodes;
    }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl p-8 mb-8">
          ...
          <div className="grid md:grid-cols-4 gap-4">
            <div className="bg-white/10 backdrop-blur rounded-lg p-4">
              ...
              <div className="text-sm text-indigo-100">Total Weeks</div>
            </div>
            <div className="bg-white/10 backdrop-blur rounded-lg p-4">
              <div className="text-3xl font-bold">{Math.round(plan.totalHours)}</div>
              <div className="text-sm text-indigo-100">Total Hours</div>
            </div>
            <div className="bg-white/10 backdrop-blur rounded-lg p-4">
            ...
            <div className="bg-white/10 backdrop-blur rounded-lg p-4">
              ...
              <div className="text-sm text-indigo-100">Current Week</div>
            </div>
          </div>
        </div>
            ...
            return (
              <div
                ...
                className={`bg-slate-800 rounded-xl overflow-hidden transition-all ${
                  status === 'in-progress' ? 'ring-2 ring-indigo-500' : ''
                }`}
              >
                {/* Week Header */}
                <div 
                  className={`p-6 cursor-pointer ${
                    status === 'completed' ? 'bg-indigo-500/10' : 
                    status === 'in-progress' ? 'bg-indigo-500/10' : ''
                  }`}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-full flex items-center justify-center ${
                        ...
                        status === 'in-progress' ? 'bg-purple-500' : 'bg-slate-700'
                      }`}>
                        {status === 'completed' ? (
                        ...
                        <div key={topic.id} className="bg-slate-900 rounded-lg p-6">
                          ...
                          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
                            {topic.activities.map((activity, actIndex) => (
                              ...
                              >
                                <activity.icon className="w-4 h-4 text-indigo-400 flex-shrink-0" />
        ...
        <div className="mt-8 bg-slate-800 rounded-xl p-6">
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <div className="text-2xl font-bold text-indigo-400">{completedTopics.size}</div>
            ...
            <div>
              <div className="text-2xl font-bold text-indigo-400">{currentWeek}</div>
...
export default LessonPlannerDB;
